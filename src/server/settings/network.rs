crate::settings!(NetworkSettings {
    /// Default starting port for player data. If UDP, this is this one of two
    ...
    /// Reset ID determines if the server has undergone a soft-reset. If this
    /// number does match the client, the client must create a new character.
    /// Used in conjunction with PlayerServerID. It is strongly advised that
    /// you backup these IDs somewhere\nMinimum=0 Maximum=2147483647
    /// Default=895534392
    ...
    /// Shows the server on the in-game browser. (Note: Steam-enabled servers
    /// are always visible in the Steam server browser)
    #[serde(with = "crate::serde::bool")]
    public: bool = false,

    /// Name of the server displayed in the in-game browser and, if applicable,
    /// the Steam browser
    ...
    /// Maximum number of players that can be on the server at one time. This
    /// excludes admins. WARNING: Server player counts above 32 will
    /// potentially result in poor map streaming and desync. Please advance
    /// with caution.\nMinimum=1 Maximum=100 Default=32
    #[serde(with = "crate::serde::u32")]
    max_players: u32 = 32,

    /// Clients must know this password to join the server. (Ignored when
    /// hosting a server via the Host button)
    password: String = format!(""),

    /// Attempt to configure a UPnP-enabled internet gateway to automatically
    /// setup port forwarding rules. The server will fall back to default ports
    /// if this fails
    #[serde(rename = "UPnP")]
    #[serde(with = "crate::serde::bool")]
    upnp: bool = true,
    ...
    /// Set the IP from which the server is broadcast. This is for network
    /// configurations with multiple IP addresses, such as server farms
